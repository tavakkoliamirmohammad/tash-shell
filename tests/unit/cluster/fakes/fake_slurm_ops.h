...
// record. Empty queue -> default-constructed return.
...
public:
    ...
    std::deque<std::vector<JobState>>        squeue_queue;
    std::deque<std::vector<PartitionState>>  sinfo_queue;
    ...
    std::vector<JobState> squeue(const std::string& cluster, ISshClient&) override {
        squeue_calls.push_back({cluster});
        if (squeue_queue.empty()) return {};
